...
const FloatingDockMobile = ({
    ...
    return (
        <div className={cn('relative block xl:hidden', className)}>
            <AnimatePresence>
                {open && (
            ...
            <button onClick={() => setOpen(!open)} className='h-10 w-10 rounded-full bg-neutral-900 flex items-center justify-center'>
...
const FloatingDockDesktop = ({
    ...
    return (
        <motion.div
            ...
            className={cn('mx-auto hidden xl:flex h-16 gap-4 items-end  rounded-2xl bg-neutral-900 px-4 pb-3', className)}
        >
            {items.map((item) => (
    ...
    return (
        <Link to={href}>
            <motion.div
                ...
                className='aspect-square rounded-full bg-neutral-800 border border-neutral-900 flex items-center justify-center relative'
            >
                <AnimatePresence>
                    {hovered && (
                        <motion.div
                            ...
                            className='px-2 py-0.5 whitespace-pre rounded-md border bg-neutral-900 border-neutral-900 text-white absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs'
                        >
                            {title}
                        </motion.div>
                    )}
